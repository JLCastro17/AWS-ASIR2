			...
			<div id='contenido'>
				<div id='tabla'>
					<table class='muse muse-border tabla'>
						...
						$result = mysqli_query($connection, "SELECT image FROM murallas WHERE nombre='$nombre'");
					...
					echo "' alt='$nombre' /></td>
						...
						<tr>
							<td><b>Estado</b></td>
							<td>";
						$result = mysqli_query($connection, "SELECT estado FROM murallas WHERE nombre='$nombre'");
						while($query_data = mysqli_fetch_row($result)) {
							echo $query_data[0];
						}
					echo "</td>
						...
						<tr>
							<td><b>Manga</b></td>
							<td>";
						$result = mysqli_query($connection, "SELECT manga FROM murallas WHERE nombre='$nombre'");
						while($query_data = mysqli_fetch_row($result)) {
							echo $query_data[0];
						}
					echo "</td>
						...
						<tr>
							...
							<td class='no-line'>";
						$result = mysqli_query($connection, "SELECT anime FROM murallas WHERE nombre='$nombre'");
						while($query_data = mysqli_fetch_row($result)) {
							echo $query_data[0];
						}
					echo "</td>
				...
				<article id='informacion' class='muse muse-border'>
					<h2>$nombre</h2>
					<br />";
						$result = mysqli_query($connection, "SELECT biografia FROM murallas WHERE nombre='$nombre'");
						while($query_data = mysqli_fetch_row($result)) {
							echo $query_data[0];
						}
					echo "<br/><hr /><br />
					<h3>Historia</h3>
					<br />";
						$result = mysqli_query($connection, "SELECT historia FROM murallas WHERE nombre='$nombre'");
						while($query_data = mysqli_fetch_row($result)) {
							echo $query_data[0];
						}
					echo "<br/><hr /><br />
